...
import { isActionTypeAllowed } from '../allowlist'
		...
		try {
			...
			for await (const text of textStream) {
				...
				if (actions.length > cursor) {
					...
					if (
						action &&
						isActionTypeAllowed(String((action as { _type?: string })._type ?? ''))
					) {
						yield {
							...action,
							complete: true,
						...
						console.warn(
							'[allowlist] dropped completed action not on server allowlist:',
							(action as { _type?: string })._type
						)
						maybeIncompleteAction = null
					}
					cursor++
				...
				if (
					...
					// Not allowlisted — do not stream to client
					maybeIncompleteAction = null
				}
			}
			...
			if (
				...
				isActionTypeAllowed(String((maybeIncompleteAction as { _type?: string })._type ?? ''))
			) {
				yield {
					...maybeIncompleteAction,
					complete: true,
